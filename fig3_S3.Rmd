...
#CHLA10 module expression Fig 3A
...
#CHLA10 module peak accessibility Fig 3B
```{r interrogate chla10}
...
#spatial module expression Fig 3E
```{r}
...
gsva
```{r}
se$total_counts<-colSums(assays(se)$count) 
ggplot(colData(se) %>% data.frame(), aes(y= total_counts, x = factor(ROILabel)))+geom_bar(stat="identity")
...
msig<-msigdbr(category = "C2", subcategory = "CGP")
kins_up<-msig[grep("KINSEY_TARGETS_OF_EWSR1_FLII_FUSION_UP", msig$gs_name),]$gene_symbol

kins_down<-msig[grep("KINSEY_TARGETS_OF_EWSR1_FLII_FUSION_DN", msig$gs_name),]$gene_symbol
...
gene_sets<-list( k1$gene,k2$gene, k3$gene)

mat<-GSVA::gsva(x, gene_sets, method = "gsva", kcdf = "Poisson", abs.ranking = F)
...
x=assays(se)$counts %>% as.matrix()
gene_sets<-list( kins_down, kins_up)
mat<-GSVA::gsva(x, gene_sets, method = "gsva", kcdf = "Poisson", abs.ranking = F)

colnames(mat)<-se$ROILabel
rownames(mat)<-c("kins_down", "kins_up")

ht2<-Heatmap(mat, col = rna_cols, column_split = factor(se$location))
