...
fus<-mat[,which(colnames(mat) %in% colnames(m1))]
fus<-fus[order(match(colnames(m1)[which(colnames(m1) %in% names(fus))], names(fus)))]

m1$fus<-0
m1$fus[which(colnames(m1) %in% names(fus))]<-fus

FeaturePlot_scCustom(m1, features = "fus", reduction = "mo_UMAP", colors_use = paletteContinuous("fireworks2"), order =T)
cds<-viewmastR::seurat_to_monocle3(m1, seu_rd = "mo_UMAP")

plot_cells(cds[,order(cds$fus, decreasing = F)], color_cells_by = "fus", cell_size = 0.7)+scale_color_gradientn(colors = paletteContinuous("fireworks"), na.value = "gray90")&NoAxes()
```
...
ggplot(df[df$Var1 == "pos",], aes(x=Var2, y = Freq , fill =Var2))+geom_bar(stat = "identity", color = "black")+scale_fill_manual(values = cols)
```
