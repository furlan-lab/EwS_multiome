...
#export c22, c23 marker genes
marker_list<-list("c22"=c22_genes, "c23" = c23_genes)


df_list<-lapply(1:length(marker_list), function(x){
  
  df<-do.call(cbind, marker_list[[x]]@assays@data@listData)
  names(df)<-names(marker_list[[x]]@assays@data@listData)
  df$gene_short_name <- marker_list[[x]]@elementMetadata@listData[["name"]]
  df$marker_gene <-ifelse( df$FDR <= 0.01 & df$Log2FC >= 1.25 , TRUE, FALSE )
  df$cluster<-names(marker_list)[x]
  df
...
df_merge<-do.call(rbind, df_list)


df_merge %>% dplyr::filter(gene_short_name == "NT5E" & Log2FC >=1.25 & FDR <= 0.01)
write.csv(df_merge, "/fh/fast/furlan_s/user/owalt/ewings/multiome/9cl/res/c22_c23_markers.csv")
```
...
#Compare to CHLA9 signatures
```{r}


chla9_genes <- getMarkerFeatures(
    ArchRProj = m2, 
    useMatrix = "GeneExpressionMatrix", 
    groupBy = "cell_line",
    bias = c( "Gex_nUMI"),
...
chla9_markers <- getMarkers(chla9_genes, cutOff = "FDR <= 0.01 & Log2FC >= 1.25")
chla9_markers@listData$CHLA9$cluster_id <- "CHLA9"


markerList<-list(chla9_markers, c22_markers, c23_markers)
names(markerList)<-c("CHLA9", "C22", "C23")
...
enrich_markers<-function(cluster_id){
  ...
  m_t2g <- msigdbr(species = "Homo sapiens",subcategory = "H") %>% 
  dplyr::select(gs_name, gene_symbol) 
  ...
  result <- enricher(gene =genes,
                     TERM2GENE = m_t2g,
                     pvalueCutoff = 0.05,
                     universe = unique(c(markerList[["CHLA9"]]@listData[["CHLA9"]]@listData[["name"]],                          markerList[["C22"]]@listData[["C22"]]@listData[["name"]], 
                     markerList[["C23"]]@listData[["C23"]]@listData[["name"]])),
                     qvalueCutoff = 0.05,
                     pAdjustMethod = "BH")
  if (length(result@result) > 0) {
    results <- result@result
...
# results_sub<-results_sub[!duplicated(results_sub$Description),]
results_sub<-results_sub %>% group_by(cluster_id)
results_sub<-results_sub%>% slice_min(pvalue, n =10)

results_sub$Description<-factor(results_sub$Description, levels =rev(unique(results_sub$Description)))
results_sub$cluster_id<-factor(results_sub$cluster_id, levels =unique(results_sub$cluster_id))

p<-ggplot(results_sub, aes(x = Description, y = cluster_id, size = Count, fill = pvalue))+
geom_point(shape = 21, color="black")+
theme_minimal()+
RotatedAxis()+
coord_flip()+
...
ggsave(p, filename="/fh/fast/furlan_s/user/owalt/ewings/multiome/9cl/Plots/chla10_c22_c23_gobp.pdf", width = 8, height = 6.5)
```
