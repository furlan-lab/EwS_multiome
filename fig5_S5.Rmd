...
se<-SummarizedExperiment(list(counts = mat), colData = out)
```
...
link_df<- read.csv("/fh/fast/furlan_s/user/owaltner/m2/res/3_subgroup_links.csv")
link_df<-link_df[!duplicated(link_df$gene),]
link_df<-link_df[!duplicated(link_df$peak),]
link_df$peak<-gsub(":", "-", link_df$peak)

link_df<-link_df[!link_df$gene %in% c(Seurat::cc.genes$s.genes, Seurat::cc.genes$g2m.genes),]
...
kmean_probes<-lapply(list(k1$gene, k2$gene, k3$gene), function(x){
  fdat[fdat$gene_short_name %in% x,]$id  
})
